...
#
################################################################
# Script:       create-deb-lxc.sh
# Purpose:      Creates a Debian 12 LXC container on Proxmox VE
# Functions:
...
[[ "$CTID" =~ ^[0-9]+$ ]] && [ "$CTID" -ge 100 ] || { echo -e "${RED}Container ID must be numeric and >= 100${NC}"; exit 1; }
